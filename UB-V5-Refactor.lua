...
function UBHubLib.Items:AddColorPicker(Config)
    -- This primitive creates a color picker button.
    -- The actual popup logic will be attached to the Window or a global overlay.
    ...
    -- In a real scenario, this returns an object that can be parented to a container.
    -- Since we don't have the Window yet, we prepare the class.
    ...
    function PickerObj:SetValue(Color)
        ...
        self.Callback(Color)
        if self.Flag then
            UBHubLib.ConfigManager:UpdateFlag(self.Flag, {R=Color.R, G=Color.G, B=Color.B})
        end
        -- Update UI visual (Button color) would happen here
    end
...
function UBHubLib:CreateWindow(Config)
    ...
    function Window:AddTab(Config)
        ...
        function TabObj:AddColorPicker(Config) return ProcessElement(UBHubLib.Items:AddColorPicker(Config), Config) end
    ...
    function Window:CreateSettingsTab()
        ...
        local function RefreshConfigs()
            ...
            if isfolder(UBHubLib.ConfigManager.Folder) then
                for _, File in ipairs(listfiles(UBHubLib.ConfigManager.Folder)) do
        ...
        local function AddThemePicker(Name, Key)
            local CurrentColor = UBHubLib.ActiveTheme.Colors[Key]
            if typeof(CurrentColor) ~= "Color3" then return end

            SettingsTab:AddInput({
                Title = Name .. " (Hex)",
                Default = CurrentColor:ToHex(),
                Callback = function(V)
                    local Success, Color = pcall(function() return Color3.fromHex(V) end)
                    if Success then
                        UBHubLib.ActiveTheme.Colors[Key] = Color
                    end
                end
            })
        end

        local ThemeKeys = {"Primary", "Secondary", "Background", "Text", "Stroke", "Divider"}
        for _, Key in ipairs(ThemeKeys) do
            AddThemePicker(Key, Key)
        end
