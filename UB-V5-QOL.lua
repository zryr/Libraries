...
-- Load ThemeManager, passing FontManager
...
local function MakeDraggable(topbarobject, object, isQuickToggle)
	...
	local inputBeganConn = topbarobject.InputBegan:Connect(function(input)
		...
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			table.insert(activeDragConnections[object], changedConnection)
		end
	end)
...
if ConfigManagerModule then
    UBHubLib.ConfigManager = ConfigManagerModule.new(UBHubLib)
else
    ...
    UBHubLib.ConfigManager = {
        new = function() return UBHubLib.ConfigManager end, Init = function() end, SetMode = function() end,
        RegisterElement = function() end, SaveSetting = function() end, LoadSetting = function(_, _, default) return default end,
        _PersistFlags = function() end, _LoadWindowSettings = function() end, _SaveWindowSettings = function() end,
        CreateConfig = function() return false end, LoadConfig = function() return false end, _ApplyFlagsToElements = function() end,
        DeleteConfig = function() return false end, OverwriteConfig = function() return false end, ExportConfig = function() return nil end,
        ListConfigs = function() return {} end, GetMode = function() return "Unavailable" end, GetCurrentConfigName = function() return "N/A" end,
        GetConfigNames = function() return {} end, GetElementByFlag = function() return nil end, SaveConfig = function() return false end,
    }
end
UBHubLib.isEditMode = false
...
function UBHubLib:CreateInternalSectionItems(parentFrameForItems, tabObjForSearch, sectionObjForSearch)
	...
	local currentWindow = UBHubLib.CurrentWindow
...
function UBHubLib:CreateSettingsTabContent(settingsTabContentFrame)
	...
	SettingsViewsContainer.Size = UDim2.new(1,0,1,-40)
	...
	local function CreateView(viewName, viewTitle)
		...
		navButton.Size = UDim2.new(0, 120, 1, 0)
...
function UBHubLib:PopulateSettingsViews(windowObject)
	...
	local customizeTabActualContentFrame = windowObject.CustomizeTabContentFrame
	...
	configItems.AddToggle({
		Title = "Config Mode (Legacy ON / Normal OFF)", Default = windowCfg:GetMode() == "Legacy", Icon = "save",
	...
	configItems.AddInput({
		Title = "New Config Name", Placeholder = "Enter name...", Icon = "file-plus-2", Callback = function(name) end, Flag = "_tempNewConfigName"
	})
	local newConfigNameInput = windowCfg:GetElementByFlag("_tempNewConfigName")

	configItems.AddButton({
		Title = "Create New Config", Icon = "plus-circle",
		Callback = function()
			local name = newConfigNameInput and newConfigNameInput.Value or ""
			if name and #name > 0 then
				windowCfg:CreateConfig(name)
				UBHubLib:MakeNotify({Title="Config", Content="Config '"..name.."' created."})
				if newConfigNameInput then newConfigNameInput:Set("") end
			else
	...
	local configDropdown = configItems.AddDropdown({
		...
		Default = currentLoadedConfig or (configNames[1] or ""), Icon = "list", Flag = "_selectedConfigToManage"
	})

	configItems.AddButton({Title = "Load Selected", Icon="download", Callback = function()
	...
	configItems.AddButton({Title = "Save Over Selected", Icon="save", Callback = function()
			if configDropdown.Value and configDropdown.Value ~= "" then
				windowCfg:SaveConfig(configDropdown.Value)
	...
	configItems.AddButton({Title = "Delete Selected", Icon="trash-2", Callback = function()
			if configDropdown.Value and configDropdown.Value ~= "" then
	...
	configItems.AddButton({Title = "Export Selected", Icon="share-2", Callback = function()
			if configDropdown.Value and configDropdown.Value ~= "" then
	...
	local themeDropdown = themingItems.AddDropdown({
		Title = "Select Theme", Options = (function()
			local opts = {}
			for _, name in ipairs(themeNames) do table.insert(opts, {Name=name, Value=name}) end
			return opts
		end)(), Default = ThemeManager.CurrentTheme.Name, Icon = "palette",
	...
	for colorName, colorValue in pairs(ThemeManager.CurrentTheme.Colors) do
		if typeof(colorValue) == "Color3" then
			themingItems.AddColorPicker({ Title = colorName, Default = colorValue, Flag = "_themeColor_"..colorName,
				Callback = function(newColor, newAlpha) ThemeManager.UpdateThemeColorValue(colorName, newColor) end
			})
		end
	end
end
...
function UBHubLib:RegisterDependency(options)
	if not options.SourceElement or not options.DependentGuiObject or not options.PropertyToChange or options.TargetValue == nil then
		warn("RegisterDependency: Missing required options.")
		return
	end
	...
	local function UpdateDependentProperty(sourceElementCurrentValue)
		...
		if options.PropertyToChange == "Visible" then
			...
			local targetFrame = options.DependentGuiObject
			...
			if shouldBeVisible then
				...
				task.delay(0.21, function()
					if targetFrame and targetFrame.Parent then
						...
						targetFrame.AutomaticSize = Enum.AutomaticSize.None
						targetFrame.Size = UDim2.new(options.OriginalSize.X.Scale, options.OriginalSize.X.Offset, 0, 0)
					end
				end)
			end
...
function UBHubLib:MakeGui(GuiConfig)
	...
	GuiConfig.Color = GuiConfig.Color or ThemeManager.GetColor("Accent")
	GuiConfig["Tab Width"] = GuiConfig["Tab Width"] or 150
	GuiConfig["SaveFolder"] = GuiConfig["SaveFolder"] or "UBV5_Settings"

	local Flags = {}
	local windowConfigManager = ConfigManagerModule.new(UBHubLib)
	windowConfigManager:Init(Flags, GuiConfig["SaveFolder"])
	UBHubLib.CurrentWindowConfigManager = windowConfigManager
	...
	ThemeManager.AddThemedObject(UBHubGui, {})

	local QuickTogglesContainer = Instance.new("Frame")
	QuickTogglesContainer.Name = "QuickTogglesContainer"
	QuickTogglesContainer.Size = UDim2.new(1,0,1,0)
	QuickTogglesContainer.BackgroundTransparency = 1
	QuickTogglesContainer.Parent = UBHubGui
	UBHubLib.QuickTogglesContainer = QuickTogglesContainer

	local DropShadowHolder = Instance.new("Frame")
	DropShadowHolder.Name = "DropShadowHolder"
	DropShadowHolder.BackgroundTransparency = 1
	DropShadowHolder.Size = GuiConfig.Size or SizeUI
	...
	SearchOverlay.BackgroundTransparency = 0.7
	SearchOverlay.Visible = false
	SearchOverlay.ZIndex = Main.ZIndex + 100
	SearchOverlay.Parent = UBHubGui
	ThemeManager.AddThemedObject(SearchOverlay, { BackgroundTransparency = 0.7})
	...
	SearchResultsFrame.Position = UDim2.new(0.5,0, 0.1, 60)
	...
	local function PerformSearch(query)
		...
		for _, elementData in ipairs(UBHubLib.SearchableElements) do
			...
			if title == lowerQuery then score = 100
			elseif title:find(lowerQuery, 1, true) then score = 75
			end
		...
		for i = 1, math.min(maxResults, #scoredResults) do
			...
			resultButton.GroupTransparency = 1
			...
			resultButton.Activated:Connect(function()
				...
				local tabContentFrame = result.Data.TabObject
				...
				if elementObject and tabContentFrame and elementObject:IsDescendantOf(tabContentFrame) then
					...
					local newCanvasY = canvasY + relativePosition - (tabContentFrame.AbsoluteSize.Y * 0.1)
					...
					elementObject.BackgroundTransparency = 0.3
	...
	local RPWidthSliderFrame = Instance.new("Frame")
	...
	local RPWidthSlider = Instance.new("Frame")
	RPWidthSlider.Name = "RPWidthSlider_Track"
	RPWidthSlider.Size = UDim2.new(1, -55, 0, 6)
	...
	RPWidthProgress.Size = UDim2.new(0,0,1,0)
	...
	RPWidthThumb.Size = UDim2.fromOffset(10,16)
	RPWidthThumb.AnchorPoint = Vector2.new(0.5,0.5)
	RPWidthThumb.Position = UDim2.new(0,0,0.5,0)
	...
	UBHubLib.ResizePanel = ResizePanel
	UBHubLib.ResizePanelControls = {
		Track = RPWidthSlider,
	...
	local minQTWidth, maxQTWidth = 30, 300
	...
	UBHubLib.UpdateResizePanelVisuals = UpdateResizePanelVisuals
	...
	RPWidthTextBox.FocusLost:Connect(function(enterPressed)
		...
		if enterPressed then
			...
			if newWidth and newWidth >= minQTWidth and newWidth <= maxQTWidth then
				...
				UpdateResizePanelVisuals(targetQT.Size.X.Offset)
			end
		else
			UpdateResizePanelVisuals(targetQT.Size.X.Offset)
		end
	...
	RPWidthSlider.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			UpdateQTWidthFromSliderMouse(input.Position)
			rpSliderDragging = true
		end
	end)
	...
	EditButton.Activated:Connect(function()
		...
		if UBHubLib.isEditMode then
			...
			if UBHubLib.QuickTogglesContainer then
				for _, qtButtonFrame in ipairs(UBHubLib.QuickTogglesContainer:GetChildren()) do
					if qtButtonFrame:IsA("Frame") and qtButtonFrame.Name:match("QuickToggle_") then
						qtButtonFrame.Selectable = false
				...
				windowConfigManager:_PersistFlags()
	...
	LayersTab.Size = UDim2.new(0, GuiConfig["Tab Width"], 1, -(Top.AbsoluteSize.Y + 5))
	...
	DecideFrame.Size = UDim2.new(0,1,1, -(Top.AbsoluteSize.Y + 10))
	...
	Layers.Size = UDim2.new(1, -(GuiConfig["Tab Width"] + 10 + 5 + 5), 1, -(Top.AbsoluteSize.Y + 10))
	...
	LayersReal.Size = UDim2.new(1,0,1,-NameTab.AbsoluteSize.Y)
	...
	StaticTabsContainer.LayoutOrder = 1000
	...
	local GuiFunc = {}
	UBHubLib.WindowObject = Main
	UBHubLib.ConfigManager = windowConfigManager
	...
	function Tabs:CreateTab(TabConfig, isStatic)
		...
		if TabConfig.Icon then
			...
			IconManager.ApplyIcon(FeatureImg, "Lucide", TabConfig.Icon)
		...
		function Sections:AddSection(SectionTitleText)
			...
			SectionFrame.Parent = ScrolLayers
			...
			IconManager.ApplyIcon(SectionToggleIcon, "Lucide", "chevron-right")
			...
			SectionLine.Size = UDim2.new(0,0,0, ThemeManager.GetSize("StrokeThickness"))
			...
			if GuiConfig["OpenSection_"..SectionTitleText] then
				SectionHeader.Activated:Fire()
			end
			local Items = {}
			local CountItem = 0
			local currentTabObject = ScrolLayers
			local currentSectionObject = SectionFrame
			function Items:AddParagraph(ParagraphConfig)
				...
				ThemeManager.AddThemedObject(ParagraphFrame, {BackgroundColor3 = "ElementBackground"})
				...
				ThemeManager.AddThemedObject(ParaPadding, { PaddingTop = "SmallPadding", PaddingBottom = "SmallPadding", PaddingLeft = "SmallPadding", PaddingRight = "SmallPadding" })
				...
				ImageAndTextFrame.Size = UDim2.new(1,0,0,0)
				...
				ImageTextLayout.VerticalAlignment = Enum.VerticalAlignment.Top
				...
				table.insert(UBHubLib.SearchableElements, {
					Title = ParagraphConfig.Title, Keywords = ParagraphConfig.Keywords or {}, Type = "Paragraph",
					Object = ParagraphFrame, TabObject = currentTabObject, SectionObject = currentSectionObject,
					OriginalFunctionObject = ParagraphFunc
				})
			...
			function Items:AddButton(ButtonConfig)
				...
				ButtonConfig.Content = ButtonConfig.Content or nil
				...
				TextFrame.Size = UDim2.new(1, -(ButtonLayout.Padding.Offset*2 + (ButtonConfig.Icon and 18 or 0) + (ButtonConfig.Content and 0 or 50) ), 1, 0)
				...
				if ButtonConfig.Dependency then
					UBHubLib:RegisterDependency({
						SourceElement = ButtonConfig.Dependency.Element, DependentGuiObject = ButtonFrame,
						PropertyToChange = ButtonConfig.Dependency.Property, TargetValue = ButtonConfig.Dependency.Value
					})
				end
				table.insert(UBHubLib.SearchableElements, {
					Title = ButtonConfig.Title, Keywords = ButtonConfig.Keywords or {}, Type = "Button",
					Object = ButtonFrame, TabObject = currentTabObject, SectionObject = currentSectionObject,
			...
			function Items:AddToggle(ToggleConfig)
				...
				ToggleConfig.Style = ToggleConfig.Style or "Switch"
				ToggleConfig.IconLib = ToggleConfig.IconLib or "Lucide"
				ToggleConfig.Icon = ToggleConfig.Icon
				...
				local ToggleFunc = { Value = ToggleConfig.Default, Dependents = {} }
				...
				if ToggleConfig.CanQuickToggle then
					...
					CreatorButton.BackgroundTransparency = 1
					IconManager.ApplyIcon(CreatorButton, "Lucide", "plus-circle")
					ThemeManager.AddThemedObject(CreatorButton, {ImageColor3 = "Icon", CornerRadius="SmallCornerRadius"})
					CreatorButton.Parent = ToggleFrame
					local CreatorButtonHighlight = Instance.new("Frame")
					CreatorButtonHighlight.Name = "CreatorButtonHighlight"
					CreatorButtonHighlight.Size = UDim2.new(1,4,1,4)
					...
					CreatorButton.Activated:Connect(function()
						...
						if ToggleFunc.IsQuickToggleCreatorActive then
							IconManager.ApplyIcon(CreatorButton, "Lucide", "check-circle-2")
				...
				ToggleSwitchFrame.LayoutOrder = 100
				...
				if ToggleConfig.Style == "Checkbox" then
					...
					ThemeManager.AddThemedObject(ToggleElement, {ImageColor3 = "Icon"})
				...
				elseif ToggleConfig.Style == "Switch" then
					ToggleElement = Instance.new("TextButton")
					...
					ToggleElement.Parent = ToggleSwitchFrame
					ThemeManager.AddThemedObject(ToggleElement, {BackgroundColor3 = "Stroke", CornerRadius="Full"})
				...
				ToggleFunc:UpdateVisuals(ToggleFunc.Value)
				function ToggleFunc:Set(newValue, suppressCallback)
					...
					for key, updateFunc in pairs(ToggleFunc.Dependents) do
						if type(updateFunc) == "function" then
							task.spawn(updateFunc, newValue)
						end
					end
				end
				ToggleElement.Activated:Connect(function()
					...
					if ToggleFunc.IsQuickToggleCreatorActive then
						if ToggleFunc.ActiveQuickToggleButton and ToggleFunc.ActiveQuickToggleButton.Parent then
							...
							QTButtonFrame.Name = "QuickToggle_" .. (ToggleConfig.Title:gsub("%s+", "_"))
							local flagBase = QTButtonFrame.Name
							...
							QTButtonFrame.Size = UDim2.new( loadedSizeXScale or 0, loadedSizeXOffset or 100, loadedSizeYScale or 0, loadedSizeYOffset or 30 )
							QTButtonFrame.Position = UDim2.new( loadedPosXScale or 0, loadedPosXOffset or 10,	loadedPosYScale or 0, loadedPosYOffset or 10 )
							QTButtonFrame.Draggable = false
							...
							MakeDraggable(QTButtonFrame, QTButtonFrame, true)
							...
							QTButton.Activated:Connect(function()
								if UBHubLib.isEditMode then
									CircleClick(QTButton, Mouse.X, Mouse.Y)
									UBHubLib.CurrentResizingQuickToggle = QTButtonFrame
									UBHubLib.ResizePanel.Visible = true
									UBHubLib.ResizePanel.Position = UDim2.new( QTButtonFrame.Position.X.Scale, QTButtonFrame.Position.X.Offset, QTButtonFrame.Position.Y.Scale, QTButtonFrame.Position.Y.Offset - UBHubLib.ResizePanel.AbsoluteSize.Y - 5 )
				...
				if ToggleConfig.Flag then
					...
					ToggleFunc:Set(savedValue, true)
				...
				if ToggleConfig.Dependency then
					UBHubLib:RegisterDependency({
						SourceElement = ToggleConfig.Dependency.Element, DependentGuiObject = ToggleFrame,
						PropertyToChange = ToggleConfig.Dependency.Property, TargetValue = ToggleConfig.Dependency.Value
					})
				end
				table.insert(UBHubLib.SearchableElements, {
					Title = ToggleConfig.Title, Keywords = ToggleConfig.Keywords or {}, Type = "Toggle",
					Object = ToggleFrame, TabObject = currentTabObject, SectionObject = currentSectionObject,
			...
			function Items:AddSlider(SliderConfig)
				...
				SliderConfig.Round = SliderConfig.Round or false
				...
				SliderFrame.Size = UDim2.new(1, 0, 0, ThemeManager.GetSize("ButtonHeight") * 1.5)
				...
				SliderTitleLabel.Size = UDim2.new(0.7,0,1,0)
				...
				SliderTrack.Size = UDim2.new(1, -(ThemeManager.GetSize("SmallPadding").Offset*2), 0, 6)
				SliderTrack.Position = UDim2.new(0, ThemeManager.GetSize("SmallPadding").Offset, 0,0)
				...
				ThemeManager.AddThemedObject(SliderThumb, {BackgroundColor3 = "Text", ImageColor3 = "Accent", CornerRadius = "Full"})
				...
				local function UpdateSliderVisuals(value)
					...
					local displayValue = SliderConfig.Round and math.round(value) or tonumber(string.format("%.2f", value))
					SliderValueLabel.Text = displayValue .. SliderConfig.Suffix
				end
				UpdateSliderVisuals(SliderFunc.Value)
				...
				if SliderConfig.Dependency then
					UBHubLib:RegisterDependency({
						SourceElement = SliderConfig.Dependency.Element, DependentGuiObject = SliderFrame,
						PropertyToChange = SliderConfig.Dependency.Property, TargetValue = SliderConfig.Dependency.Value
					})
				end
				table.insert(UBHubLib.SearchableElements, {
					Title = SliderConfig.Title, Keywords = SliderConfig.Keywords or {}, Type = "Slider",
					Object = SliderFrame, TabObject = currentTabObject, SectionObject = currentSectionObject,
			...
			function Items:AddInput(InputConfig)
				...
				InputConfig.Clearable = InputConfig.Clearable == nil and true or InputConfig.Clearable
				...
				InputFrame.Size = UDim2.new(1, 0, 0, ThemeManager.GetSize("ButtonHeight") * 1.5)
				...
				InputTextBox.Size = UDim2.new(1, InputConfig.Clearable and -22 or -4, 1, -4)
				...
				if InputConfig.Numeric then InputTextBox.TextServicePlayers = {LocalPlayer} end
				ThemeManager.AddThemedObject(InputTextBox, { TextColor3 = "Text", PlaceholderColor3 = "Text", FontFace = "Default", TextSize = "TextSize" })
				...
				if InputConfig.Dependency then
					UBHubLib:RegisterDependency({
						SourceElement = InputConfig.Dependency.Element, DependentGuiObject = InputFrame,
						PropertyToChange = InputConfig.Dependency.Property, TargetValue = InputConfig.Dependency.Value
					})
				end
				table.insert(UBHubLib.SearchableElements, {
					Title = InputConfig.Title, Keywords = InputConfig.Keywords or {}, Type = "Input",
					Object = InputFrame, TabObject = currentTabObject, SectionObject = currentSectionObject,
			...
			function Items:AddDropdown(DropdownConfig)
				...
				DropdownConfig.Default = DropdownConfig.Default or (DropdownConfig.MultiSelect and {} or nil)
				...
				DropdownConfig.MaxHeight = DropdownConfig.MaxHeight or 150
				DropdownConfig.IconLib = DropdownConfig.IconLib or "Lucide"
				DropdownConfig.Icon = DropdownConfig.Icon
				...
				local DropdownFunc = { Value = DropdownConfig.Default, Options = {}, IsOpen = false }
				local DropdownOptionCount = 0
				...
				DropdownTitleLabel.Size = UDim2.new(1, -(ButtonLayout.Padding.Offset*2 + (DropdownConfig.Icon and 18 or 0) + 20), 1, 0)
				...
				DropdownPanel.Size = UDim2.new(1,0,0, DropdownConfig.MaxHeight)
				...
				DropdownPanel.ZIndex = DropdownFrame.ZIndex + 10
				DropdownPanel.Parent = DropdownFrame
				...
				if DropdownConfig.AllowSearch then
					...
					ThemeManager.AddThemedObject(SearchBar, { BackgroundColor3 = "ElementBackground", TextColor3 = "Text", PlaceholderColor3 = "Text", FontFace = "Default", TextSize = "SmallTextSize", CornerRadius = "SmallCornerRadius" })
					...
					SearchBar.Parent = DropdownPanel
				end
				...
				ThemeManager.AddThemedObject(OptionsLayout, {Padding = "TinyPadding"})
				OptionsLayout.Parent = ScrollSelect
				local function UpdateTitleLabel()
					if DropdownConfig.MultiSelect then
						if #DropdownFunc.Value == 0 then DropdownTitleLabel.Text = DropdownConfig.Title
						...
						else DropdownTitleLabel.Text = #DropdownFunc.Value .. " selected"
						end
					else
						local selectedOptName = DropdownConfig.Title
						for _, optData in ipairs(DropdownConfig.Options) do if optData.Value == DropdownFunc.Value then selectedOptName = optData.Name break end	end
						DropdownTitleLabel.Text = selectedOptName
					end
				end
				UpdateTitleLabel()
				function DropdownFunc:Clear()
					for _, optionFrame in ipairs(ScrollSelect:GetChildren()) do if optionFrame:IsA("Frame") then optionFrame:Destroy() end end
					DropdownFunc.Options = {}; DropdownOptionCount = 0
				end
				function DropdownFunc:AddOption(optionData)
					...
					OptionFrame.Name = "Option_"..optionValue; OptionFrame.Text = ""
					...
					OptionFrame:SetAttribute("OptionValue", optionValue)
					OptionFrame:SetAttribute("OptionName", optionText)
					...
					if optionData.Icon then
						...
						OptionIcon.Name = "OptionIcon"; OptionIcon.Size = UDim2.fromOffset(14,14)
					...
					OptionLabel.Name = "OptionLabel"; OptionLabel.Text = optionText
					OptionLabel.TextXAlignment = Enum.TextXAlignment.Left; OptionLabel.BackgroundTransparency = 1
					...
					local function UpdateOptionVisual()
						local isSelected
						if DropdownConfig.MultiSelect then isSelected = table.find(DropdownFunc.Value, optionValue)
						else isSelected = (DropdownFunc.Value == optionValue) end
					...
					OptionFrame.Activated:Connect(function()
						...
						if DropdownConfig.MultiSelect then
							...
							if index then table.remove(DropdownFunc.Value, index) else table.insert(DropdownFunc.Value, optionValue) end
						else DropdownFunc.Value = optionValue; DropdownFunc:ClosePanel() end
						UpdateOptionVisual(); UpdateTitleLabel()
				...
				function DropdownFunc:Set(newValue, suppressCallback)
					DropdownFunc.Value = newValue; UpdateTitleLabel()
				...
				function DropdownFunc:RefreshOptions(newOptionsTable)
					self:Clear(); DropdownConfig.Options = newOptionsTable or {}
					for _, optData in ipairs(DropdownConfig.Options) do self:AddOption(optData) end
					self:Set(DropdownConfig.Default, true)
				end
				function DropdownFunc:OpenPanel()
					if DropdownFunc.IsOpen then return end
					DropdownFunc.IsOpen = true; DropdownPanel.Visible = true
					...
					for _, optionFrame in ipairs(DropdownFunc.Options) do
						...
						if isSelected then optionFrame.LayoutOrder = -1 else optionFrame.LayoutOrder = optionFrame:GetAttribute("CreationOrder") end
					end
					if SearchBar then SearchBar.LayoutOrder = -1000 end
					OptionsLayout:SortChildren()
				end
				function DropdownFunc:ClosePanel()
					if not DropdownFunc.IsOpen then return end
					DropdownFunc.IsOpen = false; DropdownPanel.Visible = false
					IconManager.ApplyIcon(ChevronIcon, "Lucide", "chevron-down")
				end
				DropdownButton.Activated:Connect(function() CircleClick(DropdownButton, Mouse.X, Mouse.Y); if DropdownFunc.IsOpen then DropdownFunc:ClosePanel() else DropdownFunc:OpenPanel() end end)
				if SearchBar then
				...
				if DropdownConfig.Dependency then
					UBHubLib:RegisterDependency({
						SourceElement = DropdownConfig.Dependency.Element, DependentGuiObject = DropdownFrame,
						PropertyToChange = DropdownConfig.Dependency.Property, TargetValue = DropdownConfig.Dependency.Value
					})
				end
				table.insert(UBHubLib.SearchableElements, {
					Title = DropdownConfig.Title, Keywords = DropdownConfig.Keywords or {}, Type = "Dropdown",
					Object = DropdownFrame, TabObject = currentTabObject, SectionObject = currentSectionObject,
			...
			function Items:AddColorPicker(PickerConfig)
				...
				PickerConfig.Default = PickerConfig.Default or Color3.fromRGB(255,0,0)
				...
				local PickerFunc = {Value = PickerConfig.Default, Alpha = PickerConfig.DefaultAlpha or 1}
				...
				PickerTitleLabel.Name = "PickerTitleLabel"; PickerTitleLabel.Text = PickerConfig.Title
				PickerTitleLabel.TextXAlignment = Enum.TextXAlignment.Left; PickerTitleLabel.BackgroundTransparency = 1
				PickerTitleLabel.Size = UDim2.new(1, -(PickerButtonLayout.Padding.Offset*2 + 30 + 5), 1, 0)
				...
				PickerButton.Name = "PickerButton"; PickerButton.Text = ""
				PickerButton.Size = UDim2.new(1,0,1,0); PickerButton.BackgroundTransparency = 1
				...
				local function CreateColorPickerPopup()
					...
					ColorPickerPopup.Size = UDim2.fromOffset(280, 350)
					...
					PopupTitle.Name = "PopupTitle"; PopupTitle.Text = PickerConfig.Title
					...
					SBGradBlack.Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0,1), NumberSequenceKeypoint.new(1,0)})
					...
					SBSelector.Name = "SBSelector"; SBSelector.Size = UDim2.fromOffset(12,12)
					SBSelector.AnchorPoint = Vector2.new(0.5,0.5)
					SBSelector.Position = UDim2.new(currentSaturation, 0, 1 - currentValue, 0)
					SBSelector.Image = "rbxassetid://3926305904"; SBSelector.ImageColor3 = Color3.new(0,0,0)
					SBSelector.BackgroundTransparency = 1; SBSelector.ZIndex = SaturationBrightnessFrame.ZIndex + 1
					...
					HueSliderContainer.Name = "HueSliderContainer"; HueSliderContainer.Size = UDim2.new(1,0,0,20)
					HueSliderContainer.BackgroundTransparency = 1; HueSliderContainer.Parent = ColorPickerPopup
					local HueTrack = Instance.new("Frame")
					HueTrack.Name = "HueTrack"; HueTrack.Size = UDim2.new(1, -16, 0.5, 0)
					HueTrack.AnchorPoint = Vector2.new(0.5,0.5); HueTrack.Position = UDim2.new(0.5,0,0.5,0)
					...
					HueThumb.Name = "HueThumb"; HueThumb.Size = UDim2.fromOffset(10,20)
					HueThumb.AnchorPoint = Vector2.new(0.5,0.5); HueThumb.Position = UDim2.new(currentHue,0,0.5,0)
					HueThumb.BackgroundColor3 = Color3.fromHSV(currentHue,1,1); HueThumb.ZIndex = HueTrack.ZIndex + 1
					...
					AlphaSliderContainer.Name = "AlphaSliderContainer"; AlphaSliderContainer.Size = UDim2.new(1,0,0,20)
					AlphaSliderContainer.BackgroundTransparency = 1; AlphaSliderContainer.Parent = ColorPickerPopup
					local AlphaTrack = Instance.new("Frame")
					AlphaTrack.Name = "AlphaTrack"; AlphaTrack.Size = UDim2.new(1, -16, 0.5, 0)
					AlphaTrack.AnchorPoint = Vector2.new(0.5,0.5); AlphaTrack.Position = UDim2.new(0.5,0,0.5,0)
					...
					Checkerboard.Name = "Checkerboard"; Checkerboard.Image = "rbxassetid://1696007539"
					Checkerboard.ScaleType = Enum.ScaleType.Tile; Checkerboard.TileSize = UDim2.fromOffset(10,10)
					Checkerboard.Size = UDim2.new(1,0,1,0); Checkerboard.BackgroundTransparency = 1
					...
					AlphaThumb.Name = "AlphaThumb"; AlphaThumb.Size = UDim2.fromOffset(10,20)
					AlphaThumb.AnchorPoint = Vector2.new(0.5,0.5); AlphaThumb.Position = UDim2.new(currentAlpha,0,0.5,0)
					AlphaThumb.BackgroundColor3 = ThemeManager.GetColor("Text")
					AlphaThumb.ZIndex = AlphaTrack.ZIndex + 1; AlphaThumb.Parent = AlphaTrack
					...
					InputFieldsContainer.Name = "InputFieldsContainer"; InputFieldsContainer.Size = UDim2.new(1,0,0,60)
					InputFieldsContainer.BackgroundTransparency = 1; InputFieldsContainer.Parent = ColorPickerPopup
					local IFCLayout = Instance.new("UIListLayout")
					IFCLayout.FillDirection = Enum.FillDirection.Vertical; IFCLayout.Padding = ThemeManager.GetSize("TinyPadding")
					IFCLayout.Parent = InputFieldsContainer
					local HexInputFrame = Instance.new("Frame")
					HexInputFrame.Name = "HexInputFrame"; HexInputFrame.Size = UDim2.new(1,0,0,25)
					HexInputFrame.BackgroundTransparency = 1; HexInputFrame.Parent = IFCLayout
					...
					RGBAInputFrame.Name = "RGBAInputFrame"; RGBAInputFrame.Size = UDim2.new(1,0,0,25)
					RGBAInputFrame.BackgroundTransparency = 1; RGBAInputFrame.Parent = IFCLayout
					...
					local RInput = CreateRGBAInput("R", "255"); local GInput = CreateRGBAInput("G", "255"); local BInput = CreateRGBAInput("B", "255"); local AInput = CreateRGBAInput("A", "1.0")
					AInput.Parent.Size = UDim2.new(0.28,0,1,0)
					local ActionButtonsFrame = Instance.new("Frame")
					ActionButtonsFrame.Name = "ActionButtonsFrame"; ActionButtonsFrame.Size = UDim2.new(1,0,0, ThemeManager.GetSize("ButtonHeight"))
					ActionButtonsFrame.BackgroundTransparency = 1; ActionButtonsFrame.Parent = ColorPickerPopup
					...
					local function RGBtoHex(r, g, b, a)
						r = math.clamp(math.round(r), 0, 255); g = math.clamp(math.round(g), 0, 255); b = math.clamp(math.round(b), 0, 255)
						local alphaInt = math.clamp(math.round(a * 255), 0, 255)
						if PickerConfig.NoAlpha then return string.format("#%02X%02X%02X", r, g, b) end
						return string.format("#%02X%02X%02X%02X", r, g, b, alphaInt)
					end
					local function HexToRGBA(hex)
						hex = hex:gsub("#", ""); local r,g,b,a
						if #hex == 6 then r = tonumber(hex:sub(1,2), 16); g = tonumber(hex:sub(3,4), 16); b = tonumber(hex:sub(5,6), 16); a = 255
						elseif #hex == 8 then r = tonumber(hex:sub(1,2), 16); g = tonumber(hex:sub(3,4), 16); b = tonumber(hex:sub(5,6), 16); a = tonumber(hex:sub(7,8), 16)
						else return nil end
						if r and g and b and a then return {R=r, G=g, B=b, A=a/255} end; return nil
					end
					...
					local UpdateFullColorVisuals
					local function UpdateColorFromHSV() if UpdateFullColorVisuals then UpdateFullColorVisuals() end end
					UpdateFullColorVisuals = function()
						...
						RInput.Text = tostring(math.round(baseColor.R*255)); GInput.Text = tostring(math.round(baseColor.G*255)); BInput.Text = tostring(math.round(baseColor.B*255))
						AInput.Text = string.format("%.2f", currentAlpha)
						isUpdatingInternally = false
					end
					UpdateFullColorVisuals()
					local function HandleHexInput()
						if isUpdatingInternally then return end; local rgba = HexToRGBA(HexInput.Text)
						if rgba then local h,s,v = Color3.toHSV(Color3.fromRGB(rgba.R, rgba.G, rgba.B)); currentHue, currentSaturation, currentValue, currentAlpha = h,s,v, rgba.A; UpdateFullColorVisuals()
						else UpdateFullColorVisuals() end
					end
					HexInput.FocusLost:Connect(HandleHexInput)
					local function HandleRGBAInput()
						if isUpdatingInternally then return end
						local r = math.clamp(tonumber(RInput.Text) or 0, 0, 255); local g = math.clamp(tonumber(GInput.Text) or 0, 0, 255); local b = math.clamp(tonumber(BInput.Text) or 0, 0, 255); local a = math.clamp(tonumber(AInput.Text) or 0, 0, 1)
						local h,s,v = Color3.toHSV(Color3.fromRGB(r,g,b)); currentHue, currentSaturation, currentValue, currentAlpha = h,s,v,a; UpdateFullColorVisuals()
					end
					RInput.FocusLost:Connect(HandleRGBAInput); GInput.FocusLost:Connect(HandleRGBAInput); BInput.FocusLost:Connect(HandleRGBAInput); AInput.FocusLost:Connect(HandleRGBAInput)
					ConfirmButton.Activated:Connect(function() CircleClick(ConfirmButton, Mouse.X, Mouse.Y); local finalColor = Color3.fromHSV(currentHue, currentSaturation, currentValue); PickerFunc:SetColor(finalColor, currentAlpha); ColorPickerPopup.Visible = false end)
					CancelButton.Activated:Connect(function() CircleClick(CancelButton, Mouse.X, Mouse.Y); currentHue, currentSaturation, currentValue = Color3.toHSV(initialColorOnOpen); currentAlpha = initialAlphaOnOpen; UpdateFullColorVisuals(); ColorPickerPopup.Visible = false end)
					local hueDragging = false; HueThumb.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then hueDragging = true end end); HueThumb.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then hueDragging = false end end)
					local function UpdateHueFromMouse(inputPos) if not HueTrack.AbsoluteSize.X > 0 then return end; local relativeX = math.clamp(inputPos.X - HueTrack.AbsolutePosition.X, 0, HueTrack.AbsoluteSize.X); currentHue = relativeX / HueTrack.AbsoluteSize.X; HueThumb.Position = UDim2.new(currentHue, 0, 0.5, 0); UpdateColorFromHSV() end
					UserInputService.InputChanged:Connect(function(input) if hueDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then UpdateHueFromMouse(input.Position) end end)
					HueTrack.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then UpdateHueFromMouse(input.Position); hueDragging = true end end)
					local alphaDragging = false; AlphaThumb.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then alphaDragging = true end end); AlphaThumb.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then alphaDragging = false end end)
					local function UpdateAlphaFromMouse(inputPos) if not AlphaTrack.AbsoluteSize.X > 0 then return end; local relativeX = math.clamp(inputPos.X - AlphaTrack.AbsolutePosition.X, 0, AlphaTrack.AbsoluteSize.X); currentAlpha = relativeX / AlphaTrack.AbsoluteSize.X; AlphaThumb.Position = UDim2.new(currentAlpha, 0, 0.5, 0); UpdateFullColorVisuals() end
					UserInputService.InputChanged:Connect(function(input) if alphaDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then UpdateAlphaFromMouse(input.Position) end end)
					AlphaTrack.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then UpdateAlphaFromMouse(input.Position); alphaDragging = true end end)
					local sbDragging = false; SBSelector.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then sbDragging = true end end); SBSelector.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then sbDragging = false end end)
					local function UpdateSBFromMouse(inputPos) if not (SaturationBrightnessFrame.AbsoluteSize.X > 0 and SaturationBrightnessFrame.AbsoluteSize.Y > 0) then return end; local relativeX = math.clamp(inputPos.X - SaturationBrightnessFrame.AbsolutePosition.X, 0, SaturationBrightnessFrame.AbsoluteSize.X); local relativeY = math.clamp(inputPos.Y - SaturationBrightnessFrame.AbsolutePosition.Y, 0, SaturationBrightnessFrame.AbsoluteSize.Y); currentSaturation = relativeX / SaturationBrightnessFrame.AbsoluteSize.X; currentValue = 1 - (relativeY / SaturationBrightnessFrame.AbsoluteSize.Y); SBSelector.Position = UDim2.new(currentSaturation, 0, 1 - currentValue, 0); UpdateColorFromHSV() end
					UserInputService.InputChanged:Connect(function(input) if sbDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then UpdateSBFromMouse(input.Position) end end)
					SaturationBrightnessFrame.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then UpdateSBFromMouse(input.Position); sbDragging = true end end)
				...
				PickerButton.Activated:Connect(function()
					...
					if not popupInstance or not popupInstance.Parent then popupInstance = CreateColorPickerPopup()
					else popupInstance.Visible = not popupInstance.Visible end
				...
				function PickerFunc:SetColor(newColor, newAlpha, suppressCallback)
					newAlpha = newAlpha or PickerFunc.Alpha
					PickerFunc.Value = newColor; PickerFunc.Alpha = newAlpha
					ColorPreview.BackgroundColor3 = newColor
					ColorPreview.BackgroundTransparency = 1 - newAlpha
					...
					if not suppressCallback then PickerConfig.Callback(newColor, newAlpha) end
				end
				if PickerConfig.Flag then
				...
				if PickerConfig.Dependency then
					UBHubLib:RegisterDependency({
						SourceElement = PickerConfig.Dependency.Element, DependentGuiObject = PickerFrame,
						PropertyToChange = PickerConfig.Dependency.Property, TargetValue = PickerConfig.Dependency.Value
					})
				end
				table.insert(UBHubLib.SearchableElements, {
					Title = PickerConfig.Title, Keywords = PickerConfig.Keywords or {}, Type = "ColorPicker",
					Object = PickerFrame, TabObject = currentTabObject, SectionObject = currentSectionObject,
			...
			function Items:AddDivider(DividerConfig)
				DividerConfig = DividerConfig or {}
				local height = DividerConfig.Height or ThemeManager.GetSize("StrokeThickness") or 1
				local color = DividerConfig.Color
				local transparency = DividerConfig.Transparency or 0
				...
				if type(color) == "string" then
					...
					ThemeManager.AddThemedObject(UICorner_Divider, {CornerRadius = "SmallCornerRadius"})
					UICorner_Divider.Parent = DividerFrame
					ThemeManager.AddThemedObject(DividerFrame, {})
				end
				CountItem = CountItem + 1
			end
			return Items
	...
	if customizeTabObject and customizeTabObject.ContentFrame then
		Tabs.CustomizeTabContentFrame = customizeTabObject.ContentFrame
		local settingsViews = UBHubLib:CreateSettingsTabContent(customizeTabObject.ContentFrame)
		Tabs.SettingsViews = settingsViews
	else
...
UBHubLib.ConfigManagerModule = ConfigManagerModule
